...
function OpenRoomPreference() {
  ...
  const [phases, setPhases] = useState<RegistrationPhase[]>([
    { name: 'Open Room Preferences', startDate: '', startTime: '', endDate: '', endTime: '' },
  ]);
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      setIsLoading(true);
      try {
        const response = await api.get('/adminrole/open-room-preference/');
        const now = new Date();
  ...
  const handleDelete = async (year: string) => {
    if (!confirm(`Are you sure you want to delete the Open Room Preferences event for ${displayYearMap[year]}?`)) {
      return;
    }
    try {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    console.log("Payload sent to /adminrole/open-room-preference/:", payload);
          ...
          <div className="bg-white rounded-lg shadow-md p-6">
            <form onSubmit={handleSubmit}>
              ...
              <div className="mt-8 flex justify-end">
                <button
                  type="button"
                  className="mr-4 px-4 py-2 border border-gray-300 rounded-lg shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
                >
                  Cancel
                </button>
                <button
...
export default OpenRoomPreference;
